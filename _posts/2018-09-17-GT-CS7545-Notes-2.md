...
date: 2018-09-17
timestamp: "2018-09-26 22:45:35 mama"
categories: GT
comments: true
cc: "by-nc-nd"
...
<details markdown="block"><summary style="color: red">No-Regret Happy theorem</summary>
...
+ Boosting by Majority Algorithm:
  ...
  We use EWA as the no-regret algorithm. (Note: EWA requires that $$\textbf{M} \in [0,1]^{n\times m}$$ but here $$ \textbf{M} \in \{-1,1\}^{n\times m}$$. the professor promise it will work somehow.)
...
+ Settings
  ...
  + For $$ t = 1\ldots T$$,
    ...
    + Nature select <code style="color:red"> convex function </code> $$ f_t : \mathcal{K} \mapsto \mathbb{R} $$
...
+ Online Gradient Descent Algorithm (OGD)
  ...
  + Proof
    ...
    $$ \begin{aligned}
      ...
      &\leq \sum {\nabla_t \cdot ( \textbf{x}_t - \textbf{x}^* ) } \\
      &\leq \sum { \left( \frac{1}{2\eta} \left( \| \textbf{x}_t - \textbf{x}^* \|^2 - \| \textbf{x}_{t+1} - \textbf{x}^* \|^2 \right) + \frac{\eta}{2}\| \nabla_t\|^2 \right) } \\
      &\leq \sum { \frac{1}{2\eta} \left( \| \textbf{x}_t - \textbf{x}^* \|^2 - \| \textbf{x}_{t+1} - \textbf{x}^* \|^2 \right) } + \frac{\eta}{2} TG^2 \\
      &\leq \frac{1}{2\eta} \left( \| \textbf{x}_1 - \textbf{x}^* \|^2 - \| \textbf{x}_{t+1} - \textbf{x}^* \|^2 \right) + \frac{\eta}{2} TG^2 \\
