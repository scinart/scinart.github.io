...
+ 2.1.5 引理，以下四种定义等价
  ...
  $$ ∀a∈ℝ,\, \left\{ f > a \right\} \text{可测} $$  
  $$ ∀a∈ℝ,\, \left\{ f ≥ a \right\} \text{可测} $$  
  $$ ∀a∈ℝ,\, \left\{ f < a \right\} \text{可测} $$  
  $$ ∀a∈ℝ,\, \left\{ f ≤ a \right\} \text{可测} $$

+ 2.1.11 定义 设 $$E ⊂ ℝ^d$$，$$ f : E → ℂ $$，定义$$f$$可测为

  f的实部和虚部都可测。

+ 2.1.6 引理 每个连续函数 $$ f : ℝ^d → ℝ $$ 可测
+ 2.1.12 引理 每个连续函数 $$ f : ℝ^d → ℂ $$ 可测

+ 2.1.7, 2.1.13 引理 f ~ g ⟹ g 可测

  设 $$E ⊂ ℝ^d$$可测，$$ f : E → [-∞, ∞] \text{可测}$$，f = g a.e., 则 g 可测
  设 $$E ⊂ ℝ^d$$可测，$$ f : E → ℂ \text{可测}$$，f = g a.e., 则 g 可测

+ 2.1.8 推论， f ~ 一连续函数 ⟹ f 可测

+ 2.1.14 引理 f + g 可测

  设 $$E ⊂ ℝ^d$$可测，$$ f,g : E → ℂ $$，f+g is well defined.，则 f + g 可测

  (a) 先证值域是扩充实数时，\left\{ f < g \right\}可测

  \left\{ f < g \right\} = \bigcup_{r∈ℚ}{ \left\{ f < r < g \right\} }

  (b) 再证 g + b 可测 (b 是常数)

  (c) 再证 $$ \left\{ f + g > a \right\} = \left\{ f > a - g \right\} $$ 可测
...
+ 2.1.16 引理 f*g 可测 (这里定义 0 * ∞ = 0 )
  ...
  证，先证值域是扩充实数时，$$f^2$$可测，再由 $$(f+g)^2 = f^2 + g^2 + 2fg$$，得到在扩充实数上f*g可测

  再由于复数和乘法可以由实部和虚部的乘法运算得到。所以在ℂ上f*g可测。

+ 2.1.17 引理 f/g 可测

  若f is finite a.e., g≠0 a.e., 则f/g可测

  f 是 finite a.e., 防止∞/∞，可推出 f*(1/g) = f/g a.e.,
