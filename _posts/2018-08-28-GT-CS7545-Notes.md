...
date: 2018-08-26
timestamp: "2018-09-10 23:18:41 mama"
categories: GT
comments: true
cc: "by-nc-nd"
...
+ basic inequality
  ...
  * $$ - \log (1+x) \geq -x ,\, \forall x > -1 $$
  * $$ - \log (1-x) \geq x ,\, \forall x < 1 $$
  * $$ (1+x) \leq e^x $$
  * $$ e^{\alpha x} \leq e^{\alpha \cdot 0} + (\frac{e^{\alpha \cdot 1} - e^{\alpha \cdot 0}}{1-0})x = 1 + (e^\alpha - 1) x,\,\, x \in [0,1] $$
  * $$ -log(1-x) < x + x^2 ,\,\, x \in (0, \frac{1}{2}] $$

Define Weights $$ w_i^t $$ to be the remaining weight of expert i at time t. Initially we have $$ w_1^t = 1 $$.

Say at time T, expert i predict $$ x_i^t \in \{ 0, 1 \},\,\, i = 1,\ldots,n $$.

The algorithm MWA (Majority Weight Algorithm) predicts $$ \hat{y}^t $$,

$$
\text{Algorithm MWA } :
\begin{cases}
  \hat{y}^t = \operatorname{round} \left( \frac {\sum{w_i^t}{x_i^t}}{\sum{w_i^t}} \right) \\
  w_i^{t+1} = w_i^t \left( 1 - \epsilon \right)^{𝟙\{x_i^t \neq y_1^t \}},\,\, 0 < \epsilon \geq 1
\end{cases}
$$

Define number of mistake the algorithm make at time T to be $$ M_T(MWA) = \sum_{i=1}^{T}{𝟙[\hat{y}^t \neq y^t]} $$
Define number of mistake the i-th expert make at time T to be $$ M_T(i) = \sum_{i=1}^{T}{𝟙[w_i^t \neq y^t]} $$

Define the total weight at time t to be $$ \Phi_t = \sum_{i=1}^{n}w_i^t $$
  ...
  $$ M_T(i) \left( - \log \left( 1-\epsilon \right) \right) \geq - log N - M_T(\text{MWA}) \log \left( 1-\frac{\epsilon}{2} \right) $$

  We have $$ - \log (1-\frac{\epsilon}{2}) \geq \frac{\epsilon}{2} ,\, \forall \epsilon < 2 $$ and $$ -\log(1-\epsilon) < \epsilon + \epsilon^2,\,\, \epsilon < \text{ some }0.6838 $$
...
#### Exponential Weight Algorithm
  ...
  \text{Algorithm EWA } :
  \begin{cases}
    \hat{y}^t = \frac {\sum{w_i^t}{x_i^t}}{\sum{w_i^t}} \\
    w_i^{t+1} = w_i^t \exp{ \eta l(x_i^t,y^t)} \\
  \end{cases}
  $$

  Define Loss at time T $$ L_T(EWA) = \sum {l(\hat{y}^t, y^t)}, L_T(i) = \sum{l(x_i^t, y^t)}. $$

  Define The regret of alg is $$ \text{Regret} = L_T(\text{Alg}) - \min_i{L_T(i)} $$
